...
__all__ = ["WideResNet", "create_wideresnet"]
...
def create_wideresnet(dataset="cifar10", num_classes=10):
    """
    Create WideResNet model for different datasets
    Args:
        dataset: 'mnist', 'fmnist', or 'cifar10'
        num_classes: number of classes
    """
    if dataset in ["mnist", "fmnist"]:
        # For MNIST/Fashion-MNIST: smaller ResNet with adjusted first layer
        m = models.resnet18(num_classes=num_classes)
        m.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
    else:
        m = WideResNet(depth=28, widen_factor=10, dropout_rate=0.3, num_classes=num_classes)
